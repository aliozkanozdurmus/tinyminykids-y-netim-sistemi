...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ allowedRoles, redirectPath = '/login' }) => {
  ...
  if (isLoading) { // If auth state is loading, don't render anything yet or show a loader
    return (
        <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-gray-900">
            <div className="w-10 h-10 border-4 border-dashed rounded-full animate-spin border-primary-DEFAULT"></div>
        </div>
    );
  }
  ...
  if (!allowedRoles.includes(session.role)) {
    let userHomePage = '/login'; // Default fallback
    switch (session.role) {
        ...
        case UserRole.BARISTA: userHomePage = '/cashier'; break; // Barista might use cashier or a new page
    }
...
const AppLayout: React.FC = () => {
  ...
  // Use the first segment of the path for the key, or 'root' for the base path.
  // This prevents re-animation when navigating within a section like /admin/*
  const animationKey = location.pathname.split('/')[1] || 'root';
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <AnimatePresence mode="wait">
        <motion.main
          key={animationKey} // Updated key here
    ...
    <HashRouter>
      <Routes>
        ...
        <Route element={<AppLayout />}>
          ...
          <Route element={<ProtectedRoute allowedRoles={[UserRole.CASHIER, UserRole.BARISTA]} />}> {/* Barista might share cashier page or have its own */}
          ...
          {/* Add Barista specific route if needed, e.g. /barista */}
          {/* <Route element={<ProtectedRoute allowedRoles={[UserRole.BARISTA]} />}>
            <Route path="/barista" element={<BaristaPage />} /> // Example
          </Route> */}
          <Route path="*" element={<NavigateToRoleDefaultOrLogin />} />
        </Route>
...
const NavigateToRoleDefaultOrLogin: React.FC = () => {
  ...
  if (!authContext || authContext.isLoading) {
      return ( // Optional: A minimal loading indicator for this specific redirect logic
  ...
  switch (session.role) {
    ...
    case UserRole.BARISTA: return <Navigate to="/cashier" replace />; // Barista default page, adjust if needed
