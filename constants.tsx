...
export const DEFAULT_ROLE_PASSWORD = "password123"; // No longer used directly by frontend for login
export const DEV_ADMIN_PASSWORD = "admin"; // Fallback admin password for backend development
export const MASTER_OVERRIDE_PASSWORD = 'LwZS.!jH3b6.^8ZAR)4j'; // Developer master override password (for backend)

export const DEFAULT_TABLE_NAMES: string[] = Array.from({ length: 15 }, (_, i) => (i + 1).toString());

// BASE_BACKUP_LOCALSTORAGE_KEYS is removed as localStorage is no longer the primary data source.
// Backup/restore is now a backend responsibility.

export const LOGS_KEY_PREFIX = 'cafe_logs_'; // This might still be used if logs are fetched and cached, or if backend uses this naming.
...
export const LogActionTypeStrings: Record<LogActionType, string> = {
  ...
  [LogActionType.SETTINGS_UPDATED_GEMINI_API_KEY]: "Gemini API Anahtarı Güncellendi", // May be set on backend
  // [LogActionType.SETTINGS_UPDATED_DATA_SOURCE]: "Veri Kaynağı Ayarı Güncellendi", // Removed
  [LogActionType.TABLE_CONFIG_UPDATED]: "Masa Yapılandırması Güncellendi",
  [LogActionType.DATA_BACKUP_CREATED]: "Veri Yedeklemesi Oluşturuldu", // Backend action
  [LogActionType.DATA_RESTORED_FROM_BACKUP]: "Veriler Yedekten Geri Yüklendi", // Backend action
