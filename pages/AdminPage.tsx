...
const ProductManagement: React.FC = () => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
        ...
        setFeedbackMessage({type: 'error', text: 'Ürünler yüklenirken bir hata oluştu.'});
    }
    ...
    return (
        <AdminSection title="Genel Ayarlar" icon={ICONS.settings()}>
            ...
            <div className="space-y-8">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400">
                                ...
                                (Bu ayar backend tarafından yönetiliyorsa, buradaki değişiklik bilgilendirme amaçlı olabilir.)
                            </p>
                        </div>
                         <Button
                ...
                {/* Gemini API Key and Database Settings are removed as per user request to use Docker + PostgreSQL and env vars */}
                {/* Data Backup/Restore is removed as it's now a backend responsibility with PostgreSQL */}
                 <div className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
                        Ürün açıklamaları gibi yapay zeka özelliklerini kullanmak için Gemini API anahtarı gereklidir.
                        Bu anahtar, uygulama ortam değişkenleri (`process.env.API_KEY`) aracılığıyla ayarlanmalıdır.
                        Daha fazla bilgi için <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-primary-DEFAULT hover:underline font-medium">Google AI Studio</a>'yu ziyaret edin.
                    </p>
                     {(typeof process !== 'undefined' && process.env && process.env.API_KEY) ? (
                        <p className="text-xs text-green-600 dark:text-green-400 mt-2 p-2 bg-green-50 dark:bg-green-700/30 rounded border border-green-200 dark:border-green-600">
                            Gemini API Anahtarı ortam değişkenlerinden başarıyla yüklendi.
                        </p>
                    ) : (
                         <p className="text-xs text-yellow-600 dark:text-yellow-400 mt-2 p-2 bg-yellow-50 dark:bg-yellow-700/30 rounded border border-yellow-200 dark:border-yellow-600">
                            Gemini API Anahtarı ortam değişkenlerinde (`process.env.API_KEY`) bulunamadı. AI özellikleri çalışmayabilir.
                        </p>
                    )}
                </div>
